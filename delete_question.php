...
if (isset($_GET['id']) && isset($_GET['quiz_id'])) {
    ...
    if ($user_type === 'teacher') {
        ...
        if ($check_stmt->get_result()->num_rows === 0) {
            ...
            header("Location: add_questions.php?quiz_id=$quiz_id&status=danger&msg=" . urlencode("Unauthorized access."));
            exit;
        }
        $check_stmt->close();
    }
    ...
    if ($stmt_del = $conn->prepare($sql_del)) {
        ...
        if ($stmt_del->execute()) {
            ...
            header("Location: add_questions.php?quiz_id=$quiz_id&status=success&msg=" . urlencode("Question deleted successfully."));
            exit;
        } else {
            // FAILURE: Stays on the question page
            header("Location: add_questions.php?quiz_id=$quiz_id&status=danger&msg=" . urlencode("Database error."));
            exit;
        }
    }
} else {
