...
if ($stmt_fetch = $conn->prepare($sql_fetch)) {
    ...
    if ($result_fetch->num_rows == 1) {
        ...
        if ($user_type === 'teacher' && $question['created_by'] != $user_id) {
            header("location: add_questions.php?status=danger&msg=Unauthorized+access");
            exit;
        }
    } else {
    ...
    if (!$file_upload_error) { 
        ...
        if ($stmt_update = $conn->prepare($sql_update)) {
            ...
            if($stmt_update->execute()){
                header("Location: add_questions.php?quiz_id=" . $question['quiz_id'] . "&status=success&msg=Question+Updated");
                exit;
            } else {
...
<body>
    <nav class="navbar navbar-dark bg-dark fixed-top shadow">
        <div class="container-fluid">
            ...
            <a href="add_questions.php?quiz_id=<?php echo $question['quiz_id'] ?? ''; ?>" class="btn btn-outline-light btn-sm">Exit Editor</a>
        </div>
    </nav>
                    ...
                    <div class="card-body p-4">
                        ...
                        <form action="edit_question.php?id=<?php echo $question_id; ?>" method="post" enctype="multipart/form-data">
                            ...
                            <div class="d-flex justify-content-between">
                                <a href="add_questions.php?quiz_id=<?php echo $question['quiz_id']; ?>" class="btn btn-light">Cancel</a>
